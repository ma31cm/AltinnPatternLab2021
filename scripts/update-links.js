...
const rootDir = "./source/altinndigital/_patterns";
...
allFiles.forEach(function (file) {
    ...
    if (data.includes('/patterns/')) {
        ...
        data = replaceOldLink(data);
        fs.writeFileSync(file, data);
    };
});

function replaceOldLink(data) {
    ...
    const filename = lastpart.substr(lastpart.lastIndexOf(numbers[2]) + 3, lastpart.indexOf('.') - lastpart.lastIndexOf(numbers[2]) - 3);
    const newlink = 'link.' + folder + filename;
    console.log('newlink:' + newlink);
    data = data.replace(oldlink, newlink);
    if (data.includes('/patterns/')) {
        return replaceOldLink(data);
    } else {
        return data;
    }
}
